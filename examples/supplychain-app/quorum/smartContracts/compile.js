    ...
    fs.readdir(dirname, function (err, files) { // Reading the specified folder from parameter for .sol files
      ...
      files.forEach(file => { // For each file in the folder
        ...
        if (fileName[1] === 'sol') { // Reading the file only if the extention is .sol
          data[file] = { content: fs.readFileSync(path.resolve(dirname, file), {encoding: "utf8"}) } // adding the content of file to the data object in perticular format required by the compiler
        }
      })
      resolve({ ...data })
    });
...
async function GetByteCode(numberOfIterations, dirname, entrypoint, contractName) {
  ...
  const compiledContract = output.contracts[`${entrypoint}`][`${contractName}`];
  smartContract.bytecode = compiledContract.evm.bytecode.object; // adding bytecode to the variable
  smartContract.abi = compiledContract.abi; // adding abi to the smartCOntract variable
  smartContract.gasEstimates = compiledContract.evm.gasEstimates; // adding gas estimation to the smartContract variable
  smartContract.totalCost = compiledContract.evm.gasEstimates.creation.totalCost;
